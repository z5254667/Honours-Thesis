...
class output_summary():
    def __init__(self, start_date_time, start_time, end_time, bore, model_parameters, keras_model, sklearn_model):
        ...
        # Create Files
        self.temp_general_text_path = import_functions.path(f'/{self.bore_id}/temp/', 'temp_general_text.pdf')
        self.temp_general_text_file = FPDF()
        self.temp_general_text_file.add_page()
        self.temp_general_text_file.set_font('Courier', size=8)    
    
        self.temp_input_graphs_path = import_functions.path(f'/{self.bore_id}/temp/', 'temp_input_graphs.pdf')
        self.temp_input_graphs_file = PdfPages(self.temp_input_graphs_path)
        
        self.temp_lstm_text_path = import_functions.path(f'/{self.bore_id}/temp/', 'temp_lstm_text.pdf')
        self.temp_lstm_text_file = FPDF()
        self.temp_lstm_text_file.add_page()
        self.temp_lstm_text_file.set_font('Courier', size=8)
        
        self.temp_lstm_learn_graphs_path = import_functions.path(f'/{self.bore_id}/temp/', 'temp_lstm_learn_graphs.pdf')
        self.temp_lstm_learn_graphs_file = PdfPages(self.temp_lstm_learn_graphs_path)
        
        self.temp_lstm_results_graphs_path = import_functions.path(f'/{self.bore_id}/temp/', 'temp_lstm_results_graphs.pdf')
        self.temp_lstm_results_graphs_file = PdfPages(self.temp_lstm_results_graphs_path)
        
        self.temp_svr_results_graphs_path = import_functions.path(f'/{self.bore_id}/temp/', 'temp_svr_results_graphs.pdf')
        self.temp_svr_results_graphs_file = PdfPages(self.temp_svr_results_graphs_path)
        
        self.temp_svr_text_path = import_functions.path(f'/{self.bore_id}/temp/', 'temp_svr_text.pdf')
        self.temp_svr_text_file = FPDF()
        self.temp_svr_text_file.add_page()
        self.temp_svr_text_file.set_font('Courier', size=8)

        self.temp_svr_learn_graphs_path = import_functions.path(f'/{self.bore_id}/temp/', 'temp_svr_learn_graphs.pdf')
    ...
    def create_general_text(self):
        ...
        data_range_start_date = f'{start_date_day}/{start_date_month}/{start_date_year}'
        data_range_end_date = f'{end_date_day}/{end_date_month}/{end_date_year}'

        train_set_percentage = (1 - self.model_parameters.test_size) * 100
        test_set_percentage = 100 - train_set_percentage
        periods_before = self.model_parameters.times_in - 1
        ...
        lines.append('')
        lines.append(f'Data Range: {data_range_start_date} - {data_range_end_date}')
        lines.append(f'Train Set Size: {train_set_percentage}%')
        lines.append(f'Test Set Size: {test_set_percentage}%')        
    ...
    def create_input_variable_graphs(self):
        ...
        pages = math.ceil(num_inputs/10)
        times_in = self.model_parameters.times_in
        ...
        for p in range(pages):
            ...
            while save_needed:
                try:
                    plt.savefig(self.temp_input_graphs_file, format='pdf') # Line prone to TimeoutErrors
                except TimeoutError:
                    print("TimeoutError 1 occured.")
                    save_needed = True
                else:
                    save_needed = False  
            plt.close()
        ...
        for p in range(pages):
            ...
            for i in range(plots_per_page):
                ...
                if len(self.inputs_dict[input_variables[k]] + ' - Partial Autocorrelation') >= 78:
                    font_size = 4
                else:
                    font_size = 7
        
                if plots_per_page == 1:
                    plot_acf(input_data[input_variables[k]], lags=times_in, ax=ax[0], markersize=3)
                    plot_pacf(input_data[input_variables[k]], lags=times_in, ax=ax[1], method='ywm', markersize=3)
                    ...
                    plot_acf(input_data[input_variables[k]], lags=times_in, ax=ax[i, 0], markersize=3)
                    plot_pacf(input_data[input_variables[k]], lags=times_in, ax=ax[i, 1], method='ywm', markersize=3)
        ...
        if num_combinations >= 1:
            ...
            vertical_divisions = math.ceil(optimal_rows/10)
            horizontal_divisions = math.ceil(optimal_cols/7)
            ...
            for p in range(vertical_divisions):
                ...
                for q in range(horizontal_divisions):
                    ...
                    plt.savefig(self.temp_input_graphs_file, format='pdf')
                    plt.close()
        ...
        sns.set_context(rc={'lines.linewidth': 0.5, 'lines.markersize': 0.5, 'font.size': 10, 'axes.linewidth': 0.8, 'xtick.major.size': 3, 'ytick.major.size': 3})
        self.temp_input_graphs_file.close()
    ...
    def create_lstm_text(self):
        ...
        lines.append(f'LSTM Cells: {lstm_cells}')
        lines.append(f'Fully Connected Hidden Neurons: {fc_neurons}')
        ...
        for i in range(len(training_losses)):
            if (i + 1) % (len(training_losses)//10) == 0:
        ...
        if validation_split > 0:
            ...
            for i in range(len(validation_losses)):
                if (i + 1) % (len(validation_losses)//10) == 0:
                    lines.append(f"Epoch: {i + 1}, t\ Loss: {validation_losses[i]}")    
            lines.append('')
            lines.append('')
    ...
    def create_lstm_learning_graphs(self):
        ...
        plt.savefig(self.temp_lstm_learn_graphs_file, format='pdf')
        plt.close()
    ...
    def create_result_graphs(self, model_class):
        ...
        fig.tight_layout()
        plt.savefig(active_file_dict[model_type], format='pdf')
    ...
    def create_svr_text(self):
        ...
        for i in range(len(mean_train_score)):
            if (i + 1) % (len(mean_train_score)//10) == 0:
        ...
        for i in range(len(mean_validation_score)):
            if (i + 1) % (len(mean_validation_score)//10) == 0:
    ...
    def create_log(self):
        ...
        log_file_path = import_functions.path(f'/{self.bore_id}/logs/', log_file_name)
        
        pdfs = [self.temp_general_text_path,
                self.temp_input_graphs_path, 
    ...
    def create_spreadsheet(self):
        ...
        spreadsheet_file_path = import_functions.path(f'/{self.bore_id}/spreadsheets/', spreadsheet_file_name)
        ...
        value = [self.bore_id,
                 ...
                 self.model_parameters.av_period,
                 self.model_parameters.times_in,
        ...
        for i in range(len(input_variables)):
            if i == 0:
                acf_df = pd.DataFrame({input_variables[i]: acf(input_data[input_variables[i]], nlags=max(40, self.model_parameters.times_in))})
            else:
                temp_df = pd.DataFrame({input_variables[i]: acf(input_data[input_variables[i]], nlags=max(40, self.model_parameters.times_in))})
                acf_df = pd.concat((acf_df, temp_df), axis=1)
        
        # PACF values sheet
        for i in range(len(input_variables)):
            if i == 0:
                pacf_df = pd.DataFrame({input_variables[i]: pacf(input_data[input_variables[i]], nlags=max(40, self.model_parameters.times_in))})
            else:
                temp_df = pd.DataFrame({input_variables[i]: pacf(input_data[input_variables[i]], nlags=max(40, self.model_parameters.times_in))})
        ...
        if self.model_parameters.gwl_output in ['delta', 'average_delta']:
            ...
            svr_train_df = pd.DataFrame({'scaled_train_lstm': train_prediction, 
                                          'unscaled_train_lstm': unscaled_train_prediction,
                                          'train_level_lstm': train_level_prediction})
            svr_train_df = pd.concat((train_dates, svr_train_df), axis=1)
            
            svr_test_df = pd.DataFrame({'scaled_test_lstm': test_prediction,
                                          'unscaled_test_lstm': unscaled_test_prediction,
                                          'test_level_lstm': test_level_prediction})
            svr_test_df = pd.concat((test_dates, svr_test_df), axis=1)
        else:
            svr_train_df = pd.DataFrame({'scaled_train_lstm': train_prediction, 'unscaled_train_lstm': unscaled_train_prediction})
            svr_train_df = pd.concat((train_dates, svr_train_df), axis=1)
            svr_test_df = pd.DataFrame({'scaled_test_lstm': test_prediction, 'unscaled_test_lstm': unscaled_test_prediction})  
            svr_test_df = pd.concat((test_dates, svr_test_df), axis=1)
        ...
        # def add_sheet(df, writer, sheet_name):
        ...
        #     add_sheet(general_info_df, writer, 'general_info')
        #     add_sheet(input_df, writer, 'input_data')
        #     add_sheet(scaled_input_df, writer, 'scaled_input_data')
        #     add_sheet(output_df, writer, 'output_data')
        #     add_sheet(scaled_output_df, writer, 'scaled_output_data')        
        #     add_sheet(acf_df, writer, 'acf_values')
        #     add_sheet(pacf_df, writer, 'pacf_values')
        #     add_sheet(corr_df, writer, 'correlation_values')
        #     add_sheet(lstm_train_df, writer, 'lstm_train_predictions')
        #     add_sheet(lstm_test_df, writer, 'lstm_test_predictions')
        #     add_sheet(lstm_scores_df, writer, 'lstm_scores')
        #     add_sheet(svr_train_df, writer, 'svr_train_predictions')
        #     add_sheet(svr_test_df, writer, 'svr_test_predictions')
        #     add_sheet(svr_scores_df, writer, 'svr_scores')
        #     add_sheet(lstm_history_df, writer, 'lstm_history')
        #     add_sheet(svr_history_df, writer, 'svr_history')
    ...
    def save_models(self):
        ...
        lstm_model_file_path = import_functions.path(f'/{self.bore_id}/models/', lstm_model_file_name)
        self.keras_model.model.save(lstm_model_file_path)

        # Save SVR Model
        svr_model_file_name = f'svr_model_{file_name_date_time}.pkl'
        svr_model_file_path = import_functions.path(f'/{self.bore_id}/models/', svr_model_file_name)
