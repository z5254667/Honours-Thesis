...
def kge(actual, predicted):
    ...
    return 1 - np.sqrt((cc - 1)**2 + (std_predicted/std_actual)**2 + (average_predicted/average_actual)**2)
...
# Source for a lot of these: https://agrimetsoft.com/calculators/Deviation%20of%20runoff%20Volume
...
def calculate_scores(model_class):
    ...
    model_class.test_r = r(model_class.y_train, model_class.y_hat_train)
    model_class.test_ia = ia(model_class.y_train, model_class.y_hat_train)
    model_class.test_kge = kge(model_class.y_train, model_class.y_hat_train)
    model_class.test_mbe = mbe(model_class.y_train, model_class.y_hat_train)
    model_class.test_nrmse = nrmse(model_class.y_train, model_class.y_hat_train)
