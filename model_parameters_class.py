...
class model_parameters():
    def __init__(self, epochs, times_in, out_after, other_variables, av_period=30, gwl_input='standard', gwl_output='standard'): # define the input variable names
        ...
        self.input_variables = other_variables.copy() #IS .copy() NEEDED??
        self.gwl_input = gwl_input
        self.gwl_output = gwl_output
        

        
        input_code_dict = {'standard': 'swl', 'delta': 'daily_delta_swl', 'average': 'av_swl', 'average_delta': 'av_period_delta_swl'}#,'none': 'none'}
        output_code_dict = {'standard': 'swl', 'delta': 'delta_swl', 'average': 'av_swl', 'average_delta': 'av_delta_swl'}
    ...
    def add_data(self, bore):
        ...
        if var_check_val != 0:
            self.input_dates = pd.DataFrame({'av_date': bore.av_dates})
        else:
            bore.av_period = 1
        ...
        if self.gwl_output in ['standard', 'delta']:
            ...
            self.output_current_swl = pd.DataFrame({'output_current_swl': bore.data_dict['swl']})
            #change name to include real??????
        
        #remove values not for dates
        self.output_data = self.output_data[self.times_in + self.out_after - 1:]
        self.output_data.reset_index(drop=True, inplace=True)
        
        self.output_dates = self.input_dates[self.times_in + self.out_after - 1:]
        ...
        self.output_current_swl = self.output_current_swl[self.times_in - 1:-self.out_after]
