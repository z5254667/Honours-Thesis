...
class model_parameters():
    def __init__(self, silo_variables, gwl_input='standard', gwl_output='standard'): # define the input variable names
        ...
        elif self.gwl_input == 'average':
            self.input_variables.insert(0, 'av_swl')
        elif self.gwl_input == 'average_delta':
            self.input_variables.insert(0, 'av_period_delta_swl')
        ...
        if self.gwl_output == 'standard':
            self.output_variables = 'swl'
        elif self.gwl_output == 'delta':
            self.output_variables = 'delta_swl'
        elif self.gwl_output == 'average':
            self.output_variables = 'av_swl'
        elif self.gwl_output == 'average_delta':
            self.output_variables = 'av_delta_swl'
        else:
            raise Exception("gwl_output should be 'standard', 'delta', 'average' or 'average_delta'.")
            
    def add_in_out_params(self, epochs, times_in, out_after, times_out=1, between_outs=1, av_period=30):
        if times_out < 1:
    ...
    def add_average_data(self, bore):
        if len(self.input_variables) == 0:
            raise Exception("No inputs assigned.")
            
        var_check_val = sum(list(map(lambda x: 1 if (x[0] == 'a') else 0, self.input_variables)))
        if self.output_variables[0] == 'a':
            var_check_val += 1
        
        if var_check_val != len(self.input_variables)+1 and var_check_val != 0:
        ...
        for i in range(len(self.input_variables)):
            if i == 0:
                self.input_data = pd.DataFrame({str(self.input_variables[i]): bore.data_dict[self.input_variables[i]]})
            else:
                temp_df = pd.DataFrame({str(self.input_variables[i]): bore.data_dict[self.input_variables[i]]})
                # remove first row of non delta inputs when daily delta inputs included
                if len(temp_df) < len(self.input_data): 
                    self.input_data = self.input_data[-len(temp_df):] 
                    self.input_data.reset_index(drop=True, inplace=True)
                elif len(temp_df) > len(self.input_data):
                    temp_df = temp_df[-len(self.input_data):]
                    temp_df.reset_index(drop=True, inplace=True)
                    
                self.input_data = pd.concat((self.input_data, temp_df), axis=1)
        
        if var_check_val != 0:
            # if len(bore.av_dates) > len(self.input_data): 
            #     bore.av_dates = bore.av_dates[-len(self.input_data):]
            self.dates_for_graph = bore.av_dates
        else:
            bore.av_period = 1
            self.dates_for_graph = bore.dates
                
            
        # NEW OUTPUT
        if self.gwl_output == 'standard':
            self.output_data = pd.DataFrame({'swl': bore.data_dict['swl'][self.times_in+self.out_after-1:]})
            self.output_dates = pd.DataFrame({'date' : bore.dates[self.times_in+self.out_after-1:]})
            
        elif self.gwl_output == 'delta':
            self.output_data = pd.DataFrame({'delta_swl': bore.data_dict['delta_swl'][self.times_in+self.out_after-1:]})
            self.output_dates = pd.DataFrame({'date' : bore.dates[self.times_in+self.out_after-1:]}) 
            
        elif self.gwl_output == 'average':
            self.output_data = pd.DataFrame({'av_swl': bore.data_dict['av_swl'][self.times_in+self.out_after-1:]})
            self.output_dates = pd.DataFrame({'av_date': bore.av_dates[self.times_in+self.out_after-1:]})
            
        elif self.gwl_output == 'average_delta':
            self.output_data = pd.DataFrame({'av_delta_swl': bore.data_dict['av_delta_swl'][self.times_in+self.out_after-1:]})
            self.output_dates = pd.DataFrame({'av_date' : bore.av_dates[self.times_in+self.out_after-1:]})
        
        #MANUAL CALC STUFF
        self.output_swl = pd.DataFrame({'output_swl': bore.data_dict['swl'][self.times_in+self.out_after-1:]})
        self.output_current_swl = pd.DataFrame({'output_current_swl': bore.data_dict['swl'][self.times_in-1:-self.out_after]})
                
        #DROP AS REQUIRED
        if self.gwl_input == 'delta' or self.gwl_input == 'average_delta':
            self.output_data = self.output_data.drop(index=self.output_data.index[0])
            self.output_data.reset_index(drop=True, inplace=True)
        ...
        """
        output delta swl periods AVERAGES --> MAY NOT ALIGN
        DELTA SWL IS BEING CALCULATED BEFORE AVERAGES ARE CALCULATED (CURRENTLY IN THE CODE) --> NEED TO SWITCH?
        add av to log --> update graphs
        """
